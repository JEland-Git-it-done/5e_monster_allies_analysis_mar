            ...
            if word.text.lower() in word_freq.keys():
                if len(sent.text.split(" ")) < 25:
                    if sent not in sent_score.keys():
                        ...
                        sent_score[sent] += word_freq[word.text.lower()]
    #print(sent_score)
    g = round(len(sent_list) * 0.65) #Variable that changes to ensure the summary is atleast
...
def clean_text(df_targ):
    ...
    text = str(sections[0])
    text = text.replace("Concentration required", "")
    text = text.replace("Concentration not required", "")
    return text
